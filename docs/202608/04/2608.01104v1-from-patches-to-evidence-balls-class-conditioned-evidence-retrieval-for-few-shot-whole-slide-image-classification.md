...
tags: ["query:cv"]
score: 8.0
evidence: 基于类别条件证据检索的全切片图像少样本分类
tldr: WSI分类是证据驱动的，但现有MIL方法将大量patch聚合为全局表示，在少样本下难以学习可靠聚合，且共享特征压缩了类间证据。为此提出EviBall框架，通过语义-空间分配与中心细化构建紧凑的证据球，并用类条件查询（语言或分子引导）检索证据球，生成类特定表示。在四个形态和分子任务上一致超越MIL与视觉-语言基线，同时提供空间定位的类级证据。
source: arxiv
...
motivation: 现有MIL聚合全局表示，少样本下难以可靠组织稀疏局部证据，共享特征缺乏类特异性解释。
method: 构建语义-空间证据球，用类条件查询（语言/分子引导）检索证据，形成类条件表示用于直接预测。
result: 在四个WSI任务上超越常规MIL和视觉-语言基线，并提供定位的类特定证据。
conclusion: 将少样本WSI分类重构为证据检索与类间竞争，减少对全局聚合的依赖。
---

## 摘要
全切片图像（WSI）分类是一项证据驱动的任务，其中诊断线索通常是稀疏的、空间组织的且依赖于类别的。现有的多实例学习（MIL）和视觉-语言方法将大量斑块特征聚合为单一的全局切片表示。在少样本监督下，有限的切片级标签使得学习一种可靠的聚合机制变得困难，该机制需要将稀疏的局部线索组织成紧凑且连贯的诊断证据。此外，共享的切片表示将支持候选类别及其替代类别的证据压缩到同一特征中，限制了类别特定的推理和可解释性。为解决这些问题，我们提出了EviBall，一种用于少样本WSI分类的类别条件证据检索框架。EviBall通过语义-空间分配和中心细化将局部斑块组织为证据球，在弱监督下生成紧凑且空间连贯的证据单元。然后，它使用任务特定的类别查询（包括用于形态学导向任务的语言引导查询和用于分子终点预测的分子引导查询）来检索支持性证据球，并生成类别条件的证据表示以进行直接的类别级预测。通过引入结构化证据单元和任务相关语义指导，EviBall减少了对从稀缺切片级标签中学习无约束全局聚合机制的依赖。因此，它将少样本WSI分类重新构建为结构化证据检索与候选类别之间的竞争。在四个形态学导向和分子终点WSI任务上的大量实验表明，EviBall在多种少样本设置下始终优于传统和视觉-语言MIL基线，同时为每次预测提供空间定位且类别特定的证据。
